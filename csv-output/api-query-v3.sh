...
for host_name in "${HOST_NAMES[@]}"; do
    # Create a new CSV and JSON file for each host
    output_csv="${OUTPUT_DIR}/${host_name}.csv"
    output_json="${OUTPUT_DIR}/${host_name}_groupby.json"
            ...
            for metric_file in "${METRIC_FILES_ARRAY[@]}"; do
                if [[ -f "$metric_file" ]]; then
                    while IFS= read -r metric_name; do
                        ...
                        query2_curl_command="curl -sG 'http://${ip_address}:${port}/query?pretty=true' --data-urlencode \"db=${DATABASE}\" --data-urlencode \"q=SELECT ${metric_prefix}(\\\"value\\\") FROM \\\"${metric_name}\\\" WHERE (\\\"host\\\" =~ /^${host_name}$/) AND time >= '${start_time_utc}' AND time <= '${end_time_utc}' GROUP BY time(10m) fill(none)\""

                        # Execute the curl command for query 2 and get the output
                        query2_output=$(eval "$query2_curl_command")

                        # Append the query 2 output to the JSON file
                        echo "$query2_output" >> "$output_json"
                    done < "$metric_file"
                else
                    echo "Metric file not found: $metric_file"
                fi
