...
for host_name in "${HOST_NAMES[@]}"; do
            ...
            for metric_file in "${METRIC_FILES_ARRAY[@]}"; do
                if [[ -f "$metric_file" ]]; then
                   while IFS= read -r metric_name; do
                        ...
                        query2_curl_command="curl -sG 'http://${ip_address}:${port}/query' --data-urlencode \"db=${DATABASE}\" --data-urlencode \"q=SELECT ${metric_prefix}(\\\"value\\\") FROM \\\"${metric_name}\\\" WHERE (\\\"host\\\" =~ /^${host_name}$/) AND time >= '${start_time_utc}' AND time <= '${end_time_utc}' GROUP BY time(10m) fill(none)\""
                       
                        # Get the query2 output and store it in a variable
                        query2_output=$(eval "$query2_curl_command")
                      
                        python3 image-render16.py "$query2_output"
    ...
    # Call the Python script to process the query2 results
    #python3 api-query-CR-utc9.py < "$query2_output"
done
